...
const PaginationRoot = ({ total, siblingCount = 1, page, onPageChange, children, style, className }) => {
    ...
    return (_jsx(PaginationContext.Provider, { value: paginationContextValue, children: _jsx("nav", { "aria-label": "Pagination Navigation", style: style, className: className, children: children }) }));
};
/**
...
const Trigger = ({ children, style, className, asChild = false, direction, ariaLabel }) => {
    ...
    return (_jsx("button", { "aria-label": ariaLabel || defaultAriaLabel, onClick: handleClick, disabled: isDisabled, style: style, className: computedClassName, children: children }));
};
const PaginationPrevTrigger = (props) => _jsx(Trigger, Object.assign({}, props, { direction: "prev" }));
const PaginationNextTrigger = (props) => _jsx(Trigger, Object.assign({}, props, { direction: "next" }));
const PaginationItem = ({ value, isCurrent, children, style, className, ariaLabel, asChild = false }) => {
    const context = useContext(PaginationContext);
    if (!context) {
        throw new Error("Pagination components must be used within a <Pagination.Root />");
    }
    ...
    return (_jsx("button", { onClick: handleClick, style: style, className: computedClassName, "aria-current": isCurrent ? "page" : undefined, "aria-label": ariaLabel || `Page ${value}`, role: "listitem", children: children }));
};
const PaginationEllipsis = ({ children, style, className }) => {
    const computedClassName = typeof className === "function" ? className() : className;
    return (_jsx("span", { style: style, className: computedClassName, "aria-hidden": "true", children: children }));
};
const PaginationContextComponent = ({ children }) => {
    const context = useContext(PaginationContext);
    if (!context) {
        throw new Error("Pagination components must be used within a Pagination.Root");
    }
