...
const getSizeClasses = (theme, text, icon) => ({
    leading: {
        md: {
...
export const BadgeGroup = ({ children, addonText, size = "md", color = "brand", theme = "light", align = "leading", className, iconTrailing: IconTrailing = ArrowRight, }) => {
    const colors = colorClasses[theme][color];
    const sizes = getSizeClasses(theme, !!children, !!IconTrailing)[align][size];
    const rootClasses = cx("inline-flex w-max cursor-pointer items-center transition duration-100 ease-linear", baseClasses[theme].root, sizes.root, colors.root, className);
    ...
    if (align === "trailing") {
        return (_jsxs("div", { className: rootClasses, children: [theme === "modern" && _jsx("span", { className: dotClasses }), children, _jsxs("span", { className: addonClasses, children: [addonText, isReactComponent(IconTrailing) && _jsx(IconTrailing, { className: iconClasses }), isValidElement(IconTrailing) && IconTrailing] })] }));
    }
    return (_jsxs("div", { className: rootClasses, children: [_jsxs("span", { className: addonClasses, children: [theme === "modern" && _jsx("span", { className: dotClasses }), addonText] }), children, isReactComponent(IconTrailing) && _jsx(IconTrailing, { className: iconClasses }), isValidElement(IconTrailing) && IconTrailing] }));
};
